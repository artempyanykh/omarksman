...
let doc = "Markdown LSP server for easy note-taking with cross-references and diagnostics"
...
let man =
  ...
  ; `P "Markdown LSP server for easy note-taking with cross-references and diagnostics"
...
let default_cmd =
  let term =
    ...
    @@ let+ _ = Common.term in
       `Help (`Pager, None)
  in
  let info = Term.info "marksman" ~version:"%%VERSION%%" ~doc ~sdocs ~exits ~man ~envs in
  term, info
